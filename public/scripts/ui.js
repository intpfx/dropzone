...
Events.on('display-name', e => {
	...
	$roomID.addEventListener('blur', () => {
		...
		dropzone.bug.send({ type: 'updateRID', rid: $roomID.textContent });
		$tip.textContent = '';
	});
});
...
class PeersUI {
	...
	_onPeerJoined(peer) {
		...
		setTimeout(e => globalThis.animateBackground(false), 1750); // Stop animation
	}
...
class Notifications {
	...
	_messageNotification(message) {
		if (isURL(message)) {
			...
			this._bind(notification, e => globalThis.open(message, '_blank', null, true));
		} else {
...
class NetworkStatusUI {
	...
	constructor() {
		globalThis.addEventListener('offline', e => this._showOfflineMessage(), false);
		globalThis.addEventListener('online', e => this._showOnlineMessage(), false);
